    ...
    for(int sec = 0; sec < game->speed_up_factor; sec++){
        for(int i = 0; i < game->num_agents; i++){
            ...
            if(e->agents[i]->is_tagged){
                ...
                move_heron(e->agents[i], speed, angle180(heading - 90));
            }
...
void initialize_game_starts(settings * game, episode * e){
    ...
    for(int i = 0; i < game->num_agents/2; i++){
        ...
        float hdg = angle180(0);//get_random_value(0,359));//angle180(0.0f);//get_random_value(0,359));
            e->agents[i]->pos[0] = x;
            e->agents[i]->pos[1] = y;
            e->agents[i]->heading = hdg;
            e->agents[i2]->pos[0] = game->field_width/2;
            e->agents[i2]->pos[1] = y;
...
//             int actions[2] = {get_random_value(0,17),get_random_value(0,17)};
