...
USV* create_heron(int agent_id, int team) {
...
float angle180_to_deg(float deg){
    return deg + 180;
}
...
void move_heron(USV* heron, float desired_speed, float heading_error){
    ...
    // if (heron->agent_id == 1){
    //     printf("Desired Rudder: %f MAX Rudder: %f\n", desired_rudder, heron->max_rudder);
    // }
    ...
    //printf("Prev X %f prev Y: %f Curr X %f Curr Y: %f\n", heron->prev_pos[0], heron->prev_pos[1], heron->pos[0], heron->pos[1]);
